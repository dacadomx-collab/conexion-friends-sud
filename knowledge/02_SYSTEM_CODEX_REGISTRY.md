...
- ✅ **Términos Permitidos:** `ward`, `stake`, `bio`, `showWhatsapp`, `userId`, `fullName`, `birthDate`, `country`, `state`, `city`, `invitePassword`, `newInvitePassword`, `requesterId`, `adminId`, `adminName`, `createdAt`, `plainCode`, `newStatus`, `accountStatus`, `userName`, `action`, `reason`, `departures`
- ❌ **Términos Prohibidos:** `barrio`, `estaca`, `descripcion`, `mostrarWhatsapp`, `id_usuario`, `nombre`, `fechaNacimiento`, `pais`, `estado`, `ciudad`, `municipio`, `masterPassword`, `gatePassword`, `invitationCode`, `plain_text`, `rawPassword`, `deleteUser`, `hideUser`, `deactivate`
...
| `api/account/toggle_visibility.php` | POST JSON `{userId}` | Alterna `users.status` entre `'active'`↔`'inactive'`; registra en `user_departures_log` |
| `api/account/delete_account.php` | POST JSON `{userId}` | Borrado en cascada manual + `unlink()` de fotos + registro `user_departures_log action='deleted'` |
| `api/admin/get_departures.php` | GET `?requesterId=` | Lista `user_departures_log` DESC — solo `admin` |
| `api/admin/add_whitelist.php` | POST JSON `{phone, requesterId}` | Sanitiza el teléfono a dígitos puros e inserta en `whitelist_phones`; HTTP 409 si duplicado — solo `admin` |
| `api/admin/get_whitelist_history.php` | GET `?requesterId=` | JOIN `whitelist_phones + users` devuelve historial de altas ordenado por `created_at DESC` — solo `admin` |
| `api/admin/import_whitelist_csv.php` | POST multipart `{csv: File, requesterId: int}` | Importa CSV masivo: sanitiza teléfonos, inserta en `whitelist_phones` + actualiza `profiles.group_join_date` si el usuario ya existe — solo `admin` |
...
| :--- | :--- | :--- |
| 1 | KPI Cards — 5 tarjetas: Miembros / Activos / Admins / Ocultos / Eliminados | Arriba |
| 2 | `InvitationPasswordAdmin` (gestión de contraseña de invitación) | En medio |
| 3 | **Gestión de Lista Blanca (Acceso)** — formulario + historial reciente + modal "Ver todos" | Tercero |
| 4 | Lista de Usuarios Registrados (buscador + lista paginada) | Cuarto |
| 5 | **Bajas / Ocultos** — registro `user_departures_log` (`id="seccion-bajas"`) | Abajo |

#### Bloque 3: Gestión de Lista Blanca
- Formulario: `Input` con placeholder `"Ej. +52 555 123 4567"` + helper text sobre sanitización + botón "Agregar Número".
- Sanitización: `preg_replace('/[^0-9]/', '', $phone)` — solo dígitos al backend.
- Historial reciente: tabla de últimos 5 registros (Número, Fecha, Admin que lo agregó).
- Modal "Ver todos": `<Dialog>` de shadcn/ui con tabla completa y scroll. Botón "Ver todos los registros" en cabecera.
- Estado cliente: `whitelistPhone` (string input), `whitelistAdding` (bool), `whitelistError` / `whitelistSuccess` (string|null), `whitelistHistory` (WhitelistEntry[]), `whitelistModalOpen` (bool).
- Endpoints: `POST api/admin/add_whitelist.php` · `GET api/admin/get_whitelist_history.php`.

#### KPIs — 5 tarjetas (grid responsivo)
| KPI | Fuente | Icono | Color |
| :--- | :--- | :--- | :--- |
| Miembros | `users.length` | `Users` | neutro |
| Activos | `users.filter(status='active')` | `UserCheck` | emerald |
| Admins | `users.filter(role='admin')` | `Shield` | primary |
| Ocultos | `users.filter(status='inactive')` | `EyeOff` | slate — **clickeable → scroll** |
| Eliminados | `departures.filter(action='deleted')` | `UserX` | destructive — **clickeable → scroll** |

- Grid: `grid-cols-2 sm:grid-cols-5` — 2 columnas en móvil (layout 2+2+1), 5 en desktop.
- Las tarjetas "Ocultos" y "Eliminados" llaman a `scrollToBajas()` al hacer clic.
- **No hay endpoint dedicado de stats** — todos los valores se computan en el `useMemo kpis` a partir del estado ya cargado (`users` + `departures`), sin llamada extra al backend.

#### Bloque 4: Bajas / Ocultos
- Consume `GET api/admin/get_departures.php?requesterId={id}`.
- Estado: `departures: DepartureEntry[]`, `loadingDepartures`, `departuresError`.
- Muestra fila por entrada: icono (`EyeOff` para `hidden`, `UserX` para `deleted`), nombre, badge de acción, fecha formateada.
- Separador tiene `id="seccion-bajas"` y `scroll-mt-20` para compensar el header sticky.
- `scrollToBajas()` usa `document.getElementById("seccion-bajas")?.scrollIntoView({ behavior: "smooth" })`.
- Propósito: ministración — el liderazgo puede saber quién se fue y cuándo.
...
| Teléfono Permitido | `phone` | `phone` | String (solo dígitos) |
| Ya se registró | `is_used` | `isUsed` | Bool (0/1) |
| Nombre de referencia | `reference_name` | `referenceName` | String (nullable, del CSV) |
| Agregado por | `added_by` | `addedBy` | Int (FK users.id, nullable) |
...
- `phone`: VARCHAR(30) UNIQUE — Número solo-dígitos (sanitizado con `preg_replace('/[^0-9]/', '')`) autorizado para registrarse
- `is_used`: TINYINT(1) DEFAULT 0 — 0 = disponible para registro; 1 = ya utilizado
- `added_by`: INT NULL — FK → `users.id` ON DELETE SET NULL | Admin que agregó el número
- `reference_name`: VARCHAR(150) NULL — Nombre de referencia del futuro miembro (del CSV de importación, columna `Nombre_Referencia`)
- `created_at`: TIMESTAMP DEFAULT CURRENT_TIMESTAMP — Fecha y hora de alta en la lista

> ⚠️ **Migración pendiente:** `ALTER TABLE whitelist_phones ADD COLUMN reference_name VARCHAR(150) NULL AFTER is_used;`
...
> Números nuevos se agregan uno a uno desde el Dashboard Admin vía `api/admin/add_whitelist.php`; o masivamente desde CSV vía `api/admin/import_whitelist_csv.php` (solo admins).
> El CSV espera columnas: teléfono, `Nombre_Referencia`, `fECHA DE iNGRESO`, `Insignia`. La columna `Insignia` es informativa y no se almacena (se computa automáticamente en frontend).
> La columna `fECHA DE iNGRESO` del CSV se usa para actualizar `profiles.group_join_date` en usuarios que ya se hayan registrado con ese teléfono.
> Eliminaciones se registran en `whitelist_audit_log` vía `api/delete_whitelist_phone.php`.
> El historial de altas se consulta vía `api/admin/get_whitelist_history.php`.
...
### Tabla: `user_departures_log` *(Migración 07 — ejecutar `database/migracion_07_user_departures.sql`)*
- `id`: INT UNSIGNED AUTO_INCREMENT — Clave primaria
- `user_name`: VARCHAR(150) NOT NULL — Nombre del usuario **al momento de la acción** (persiste aunque el usuario sea eliminado)
- `action`: ENUM('hidden','deleted') NOT NULL — `hidden` = ocultó cuenta; `deleted` = eliminó cuenta permanentemente
- `reason`: TEXT NULL — Razón opcional proporcionada por el usuario al momento de la acción
- `created_at`: TIMESTAMP DEFAULT CURRENT_TIMESTAMP

> **Regla de negocio — hidden:** Al ocultar la cuenta, `users.status` cambia a `'inactive'` y se inserta una fila con `action='hidden'`. Al reactivar, la fila `'hidden'` más reciente de ese usuario se elimina del log para reflejar la reversión. El nombre se guarda como texto plano (no FK) para que persista si la cuenta se borra después.
> **Regla de negocio — deleted:** Antes de iniciar el borrado en cascada, el nombre del usuario se inserta con `action='deleted'`. No tiene FK a `users.id` deliberadamente, para que el registro sobreviva al borrado.
> **Propósito:** Ministración — los administradores pueden consultar quién ocultó o eliminó su cuenta y la fecha exacta, para fines de seguimiento pastoral.
> **Script:** `database/migracion_07_user_departures.sql`